...
export default function CritiquesGrid({ critiques }: CritiquesGridProps) {
  ...
  const filtered = useMemo(() => {
    let result =
      filter === "all"
        ? critiques
        : critiques.filter((c) => c.type === filter);
            ...
            <AnimatePresence>
              {showFilterMenu && (
                <motion.div
                  ...
                  className="absolute right-0 z-10 mt-1 min-w-[120px] overflow-hidden rounded-md border border-border bg-card shadow-sm"
                >
                  {typeFilters.map((f) => (
                    <button
                      key={f.value}
            ...
            <AnimatePresence>
              {showSortMenu && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -4 }}
                  transition={{ duration: 0.15 }}
                  className="absolute right-0 z-10 mt-1 min-w-[100px] overflow-hidden rounded-md border border-border bg-card shadow-sm"
                >
                  {sortOptions.map((s) => (
                    <button
                      key={s.value}
